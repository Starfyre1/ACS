...
public class HeaderDisplay extends TKTitledDisplay implements FocusListener, ActionListener {
	...
	protected Component createDisplay() {
		...
		mCharacterNameField = new JTextField(CharacterSheet.FIELD_SIZE_EXLARGE1);
		mCharacterNameField.addFocusListener(this);

		JLabel playerNameLabel = new JLabel(PLAYER_NAME_LABEL, SwingConstants.RIGHT);
		mPlayerNameField = new JTextField(CharacterSheet.FIELD_SIZE_EXLARGE1);
		...
		mCampaignButton = JournalRecord.getDateButton(CampaignDateChooser.getCampaignDate(), new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				CampaignDateChooser cal = new CampaignDateChooser(characterSheet.getFrame(), CampaignDateChooser.parseCampaignDate(CampaignDateChooser.getCampaignDate()));
				String date = cal.getSelectedDate();
				if (!(date.isEmpty() || date.equals(CampaignDateChooser.getCampaignDate()))) {
					CampaignDateChooser.setCampaignDate(date);
					mCampaignButton.setText(cal.getDisplayableDate());
					characterSheet.getJournalTab().gameDayStartQuestion();
					characterSheet.fireDateUpdated(date);
				}
			}
		});
